...
class ModelClass
{
	// VertexType은 색상(color) 요소를 텍스처 좌표(texture coordinate) 요소로 대체했습니다.
	//
	// VertexType의 이러한 변경은 단순한 색상 모델에서 텍스처가 적용된 모델로 전환하기 위한
	// 핵심적인 단계입니다.
	//
	// 이전에는 각 정점(vertex)에 색상 값이 직접 할당되어 폴리곤 면에 색상이 보간(interpolate)되었지만,
	// 이제는 float2 타입의 텍스처 좌표를 사용해 텍스처 이미지의 특정 지점(UV 좌표)을
	// 각 정점에 연결합니다.
	//
	// 이를 통해 GPU는 정점 위치와 텍스처 좌표를 이용해 텍스처를 폴리곤에 "매핑"할 수 있으며,
	// 하나의 폴리곤에 고해상도의 상세한 이미지를 표현할 수 있게 됩니다.
	// 이는 3D 그래픽스에서 텍스처링의 기본 원리인 UV 매핑의 핵심입니다.
	struct VertexType
	{
		XMFLOAT3 position;
		XMFLOAT2 texture;
